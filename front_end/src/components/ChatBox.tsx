...
const DEFAULT_CHAT: ChatItem[] = [
  {
    role: "tip",
    message: "Hi, I'm AO Agent! Ask me anything below — let's get started!",
    timestamp: Date.now(),
  },
];
...
// Use aoconnect result to wait for message processing completion
const pollForResult = async (reference: string): Promise<{ data: string; attestation?: any; reference?: string; status?: string }> => {
  try {
    ...
    // Compatible with different structures: prioritize parsing status and response (response is JSON string)
...
const ChatBox: React.FC<ChatBoxProps> = ({ onAttestationUpdate }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Use dryrun to query inference results by reference
